...
Last Updated: September 18, 2026 (KYR5-b set-id-only UPC backfill chunk 4 — 34 set-id-only rows filled; barcode-only gap 523; set-id-only remaining 486; online-only remaining 13; in-store 24 untouched; do not reopen #121–#193)
...
- Rows missing UPC/EAN only (may still have set-id): 523 — KYR5-b barcode-only gap after Job 2 chunk 4 (34 set-id-only UPCs; chunk 3 filled 21; chunk 2 filled 25; chunk 1 filled 42). Set-id-only remaining 486.
...
3) JOB 2 SET-ID-ONLY — chunk 4 filled 34 exact-pack UPC/EANs (MediNatura brand-site + DailyMed carton for Genexa / Biofreeze / Mineral Ice Extreme Gel / Sumifun; Topricin Classic 4 oz brand barcode; Theraflu daytime powder Target PDP). Remaining 486. Chunk 1–3 skip list still dry (Tylenol RS hospital blister NDC GTIN; Member’s Mark ES APAP; Walgreens ES dyes+talc; Dimetapp FPO; Assured Headache PM ValuHealth; Amazon day/night combo + Equate kids day+night sleeve; Sudafed PE kids check fail; CVS/Equate kids DPH FPO/zeros; Amazon dual-action / L484 pouch check fail; Coldcalm; DG mucus-DM ER / dyed APAP check fail; Tylenol For Children Plus Adults must not steal ES-liquid UPC; B&T nighttime 308079* older Mexico / barley-malt family; B&T Psoriaflora NDC-as-GTIN only; Chestal Honey / Arnicare PEG must not steal existing shea/honey twin UPCs). NDC-as-GTIN rejected.
