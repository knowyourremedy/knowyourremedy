...
Last Updated: September 17, 2026 (store PDPs first-class for OI + UPC/EAN — sprouts.com / Walmart / Target / CVS / Amazon / grocery house; exact pack only; store generics hunt on write; missing-barcode report 87 no-code / ~695 barcode-only after Boiron in-store; do not reopen #121–#185; Thrive no_OI closeout written — batch64)
...
- KYR5 paused after PR #174 (resource-exhausted). KYR5-b in-store Boiron opened. Still missing ~695 after this chunk (many dry). Do not reopen #121–#185. Does not grade. Does not invent UPCs. Does not open new product rows unless Brandon says so. Attach real codes from carton photo / PDP carousel / DailyMed / brand site onto existing Search rows. OI hunt order already on notes (carousel + Amazon + Google) also applies to finding a UPC on a label photo. GitHub Merge ≠ git pull. One PR at a time.
- KYR5-b is the barcode catch-up bot. Same rules as KYR5. Do not reopen #121–#185. In-store Boiron opened. Still missing ~695 after this chunk (many dry).
...
- Carton photo = polish.
- ~695 rows on main still need barcode backfill after KYR5-b in-store Boiron, many dry (KYR5 / KYR5-b). Forward catalog (KYR3) attaches UPC when the source has it.
...
- Store PDPs are first-class sources — sprouts.com, Walmart, Target, CVS, Amazon, grocery house sites. Use them for Other Ingredients AND UPC/EAN. Exact pack only (brand + name + form + strength + count). Specs / details / carousel / barcode tile. HTML bullets alone ≠ missing. No UPC on the page ≠ invent one. Next catalog pile (store generics) uses this hunt on write, not a later factory.
...
Next session
- LIVE NOW: (1) Thrive no_OI stash CLOSEOUT DONE (batch64) (2) Store generics (3) Amazon 3P brands not on main. KYR5 paused after PR #174 (resource-exhausted). KYR5-b is the barcode catch-up bot. Same rules. Do not reopen #121–#185. KYR5-b in-store Boiron opened. Still missing ~695 (many dry).
...
- KYR5 paused after PR #174 (resource-exhausted). KYR5-b is the barcode catch-up bot. Same rules. Do not reopen #121–#185. KYR5-b in-store Boiron opened. Still missing ~695 (many dry). OI hunt order already on notes (carousel + Amazon + Google) also applies to finding a UPC on a label photo.
...
- Night barcodes = KYR5 paused after PR #174 (resource-exhausted). KYR5-b is the barcode catch-up bot. Same rules. Do not reopen #121–#185. KYR5-b in-store Boiron opened. Still missing ~695 (many dry). OI hunt order already on notes (carousel + Amazon + Google) also applies to finding a UPC on a label photo.
...
- Next catalog day: store generics, then Amazon 3P. Thrive no_OI CLOSEOUT DONE (batch64). KYR5 paused after PR #174 (resource-exhausted). KYR5-b is the barcode catch-up bot. Same rules. Do not reopen #121–#185. Sprouts house stays STASHED. Do not catalog Sprouts.
...
Totals
- Search rows: 2203
- Rows with a code: 2116
- Rows missing UPC/EAN/set-id: 87
- Rows missing UPC/EAN only (may still have set-id): 695 — this is the KYR5-b barcode-only gap after in-store Boiron
- Unique formulaId 1725 vs raw 2203 (478 pack-size clones). Missing-code set is 87 rows / 87 formulaIds — no pack-size clones in the no-code pile.

1) IN-STORE — 52 missing (group by brand). Hunt these first.
