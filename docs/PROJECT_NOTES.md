...
Last Updated: July 12, 2026 (app UI & design system v1 locked)
...
Synthetic dyes are ALREADY High-tier (Methodology §5): Red 40, Yellow 5/6, Blue 1/2, Green 3 — any one → Avoid. Do NOT re-rate any of them as moderate. Rationale leads with independent science (EU hyperactivity warning labels / Southampton study, plus state-level bans); FDA action is corroboration only.
...
NEXT SESSION: lock this rating-record schema into concrete types once, then resume the verdict pass capturing straight into that shape.
...
================================================================ 6. APP UI & DESIGN SYSTEM (v1 — locked July 12, 2026)
The post-scan product-detail screen is the app's centerpiece; it presents the two-layer methodology (verdict + per-ingredient proof). Built in a deliberately plain, Yuka-clean idiom. (Mockups from this session: post-scan screen, cleaner-options page, browse grid.)
...
Header: catalog product image + product name (Playfair serif — the one KYR signature flourish, everything else is clean sans) + brand + verdict.
Verdict: understated — a colored status dot + the verdict WORD. No 0–100 score (Methodology §3).
Two sections: FLAGGED (the concerns) and CLEARED (the fine stuff) — our analog to Yuka's Negatives/Positives.
Each row: line-icon · bold title · one gray reason line · status dot · chevron. ALL proof (sources + detail) lives behind the chevron drill-down, keeping the top level calm — you only see detail on tap. This is the key move that keeps it readable.
Tabs up top: Overview · Ingredients · Photos.
...
BRAND green (
#2d4a3e) = tappable/actionable things ONLY (scan button, "See all", active nav, links, the saved star). Always means "you can act here." The verdict word is colored in its status color (the one bit of rating-color emphasis in the header). Product photos carry the visual warmth — never colored UI chrome. Rare exception allowed: a colored row-icon for an active-safety cap (e.g., colloidal silver), if it genuinely needs to stand out — start conservative.
...
Cleaner options (clean alternatives)
...
The single best swap shows INLINE on the result screen, below the breakdown (NOT hidden in a tab). A "See all cleaner options" button opens the full ranked list (its own page).
Ranking rule (LOCKED): same medicine / closest analog first → then weighted toward what's easy to find where the user already shops (same-store availability) → specialty / harder-to-find last.
...
Per-category folder pattern (this is what's live; first surface of the rating data): app/clean-picks/page.tsx (landing) · pain-fever/page.tsx · cold-flu/page.tsx · allergies/page.tsx components/clean-picks/ PickCard.tsx · RetailerChips.tsx · CategoryTile.tsx lib/clean-picks/ classColors.ts · painFeverPicks.ts (defines Pick type + ClassKey union) · coldFluPicks.ts · allergyPicks.ts
...
Worwood-grounded essential-oil safety reference. Frame toward caution/safety, not ingestion. Rated on cleanliness; no efficacy claims. Status colors: External Only / Dilute First / Internal Only. Current wiring: reads oil data from lib/medsData.js (oils in the essential_oils category) and app/oils/[slug]/page.tsx uses DosageCalculatorIcon as a placeholder. Clean-up task: give oils their own data file (lib/oilsData.js) + a fitting icon, THEN medsData.js + DosageCalculatorIcon.tsx can be removed.
...
NEXT SESSION: lock the rating-record schema into concrete types (Section 5 fields, incl. the new productImage / userPhotos[] / cleanAlternatives), then resume the verdict pass capturing straight into it.
...
App UI (design system v1 locked — §6; build when we move from data to app)

Post-scan screen, Flagged/Cleared drill-down, cleaner-options (inline + full page), catalog-first image pipeline, saved-star behavior.
Oil Library
...
Attorney review (Terms, Privacy, Medical Disclaimer, auto-renewal + affiliate disclosure + user-photo UGC) via Richard Aaron. LLC formation. Reconcile footer Legal links (/disclaimer, /affiliate-disclosure, /about don't exist; /terms + /privacy are live). Update Footer copyright year. Final QA.
Phase 2
...
July 12 — App UI & design system v1 locked (post-scan screen)
...
May 26 — Cold & Flu Clean Picks (13) + editorial standard locked (seed oils, TiO2, FD&C dyes disqualifying). Discoveries: Sudafed/Mucinex/Maty's rejected; Equate Mucus ER + Badger as clean equivalents. Advil removed from Pain & Fever (→ 9 picks). May 26 — Pain & Fever Clean Picks shipped; per-category folder architecture locked; Card design Option A. May 25 — Clean Brands → Everyday Clean Picks rename (/brands → /clean-picks). Scope locked to mainstream retailers. May 22 — Visual direction locks (vintage scientific plates, Blue B section label, Worwood internal-use gate). Earlier — Conditions rename, interaction checker, dosage calculator, Supabase schema. [most now cut]
