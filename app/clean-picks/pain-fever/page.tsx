...
export default function PainFeverPage() {
  const [activeFilter, setActiveFilter] = useState<FilterKey>('top');

  const oralPicks = PAIN_FEVER_PICKS.filter(p => p.form === 'oral');
  const topicalPicks = PAIN_FEVER_PICKS.filter(p => p.form === 'topical');
  const sublingualPicks = PAIN_FEVER_PICKS.filter(p => p.form === 'sublingual');
  const topPicks = PAIN_FEVER_PICKS.filter(p => p.topPick);
