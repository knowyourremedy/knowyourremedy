...
type FilterKey = 'top' | 'oral' | 'nasal' | 'eye';

export default function AllergiesPage() {
  const [activeFilter, setActiveFilter] = useState<FilterKey>('top');

  const oralPicks = ALLERGY_PICKS.filter(p => p.form === 'oral');
  const nasalPicks = ALLERGY_PICKS.filter(p => p.form === 'nasal');
  const eyePicks = ALLERGY_PICKS.filter(p => p.form === 'eye');
  const topPicks = ALLERGY_PICKS.filter(p => p.topPick);

  const FILTERS: { key: FilterKey; label: string; count: number }[] = [
    { key: 'top', label: '⭐ Top Picks', count: topPicks.length },
    { key: 'oral', label: 'Oral', count: oralPicks.length },
    { key: 'nasal', label: 'Nasal', count: nasalPicks.length },
    { key: 'eye', label: 'Eye', count: eyePicks.length },
  ];

  let visiblePicks: Pick[] = [];
  if (activeFilter === 'top') visiblePicks = topPicks;
  else if (activeFilter === 'oral') visiblePicks = oralPicks;
  else if (activeFilter === 'nasal') visiblePicks = nasalPicks;
  else if (activeFilter === 'eye') visiblePicks = eyePicks;
    ...
    <main style={{
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{
          ...
          {ALLERGY_PICKS.length} verified clean picks for sneezing, congestion, itchy eyes, and seasonal allergies.
        </p>
      </div>
      ...
      {activeFilter === 'top' && (
        <div style={{
          ...
          <div style={{ fontSize: '0.88rem', color: '#2a3a34', lineHeight: 1.6 }}>
            <span style={{ fontWeight: 600, color: '#2d4a3e' }}>Our 3 picks across oral, kids, and nasal.</span> Chosen for ingredient quality and availability at stores you can actually get to.
          </div>
        </div>
      )}
