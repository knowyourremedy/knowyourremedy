...
// ─── Data: all categories and remedies in one structure ─────
const CATEGORIES = [
  {
...
export default function Remedies() {
  ...
  // Filter remedies by search term — case-insensitive, matches name OR category
  const filtered = useMemo(() => {
    ...
    return CATEGORIES
      .map(cat => {
        // If the category name itself matches, include all its remedies
      ...
      <section style={{
        ...
        {search && (
          <p style={{
            ...
            {totalResults === 0
              ? `No remedies match "${search}"`
              : `${totalResults} remed${totalResults === 1 ? 'y' : 'ies'} matching "${search}"`}
          </p>
        )}
      </section>
        ...
        {filtered.length === 0 ? (
          <div style={{
            ...
            <p style={{ fontSize: '0.88rem', lineHeight: 1.5, maxWidth: '380px', margin: '0 auto' }}>
              Try a different search term, or clear the search to see all remedies.
            </p>
          </div>
        ) : (
